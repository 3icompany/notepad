...
(function ($, window, document, undefined) {
  ...
  async function uploadTempFile(url) {
    ...
    var res = await fetch(
      `https://dieuhanh.vatco.vn/MobileLogin/CreateTempFileFromUrl?url=${url}`,
      requestOptions
    );
  ...
  async function removeTempFile(url) {
    ...
    var json = await fetch(
      `https://dieuhanh.vatco.vn/MobileLogin/DeleteFileTemp?filePath=${url}`,
      requestOptions
    );
    var rs = json.json();
  }
  ...
  function loadData888(data) {
    ...
    layerStorage.forEach((layer, index) => {
      ...
      li.innerHTML = `<img src="assets/images/notepad/layer/layer-${
        index + 1
      }.png">`;
  ...
  function addNewMeeting(name, id) {
    ...
    joinButton.onclick = function () {
      jQuery.ajax({
        type: "POST",
        url:
          "https://dieuhanh.vatco.vn/MobileLogin/GetZoomSdkMeetingSchedule?id=" +
          id,
        contentType: "application/json",
        dataType: "JSON",
        success: function (rs) {
  ...
  $(function () {
    ...
    function renderWorkSheetPagination(page, pos, val, prev, length, num) {
      ...
      for (var i = 0; i < 5; i++) {
        ...
        if (i === 0) {
          li = $(`
                    <li data-next='-1' data-pos=${i} data-page='${
            page - 1
          ...
          li = $(`
            ...
            pos === 3 && page === Math.ceil(num / length) ? "disable" : ""
          }">
                        <a class="page-link" href="#" aria-label="Previous">
          ...
          li = $(`
                    <li data-next='0' data-pos=${i} data-page='${
            page + i - 1
    ...
    $("#btn_login").on("click", function () {
      if ($("#username")[0].value != "" && $("#pass")[0].value != "") {
        ...
        //vuong
        fetch(
          "https://dieuhanh.vatco.vn/MobileLogin/LoginNoCheckOnlineNodeJs",
          requestOptions
        )
          .then((response) => response.json())
          .then(async (result) => {
    ...
    socket.on("loginRoom", ({ room, userID, socketIDUser }) => {
      ...
      document.getElementById(
        "login_room"
      ).innerHTML = `<div id="" class="modal" style="display: flex;justify-content: center;height: 120px;" >
    ...
    $("#logout").on("click", function () {
      ...
      fetch(
        `https://dieuhanh.vatco.vn/MobileLogin/LogOutNodeJs?userName=${
          $("#username")[0].value
        }`,
        requestOptions
      )
    ...
    function getListUserOnline() {
      ...
      fetch(
        "https://dieuhanh.vatco.vn/MobileLogin/GetListUserOnlineNotepad",
        requestOptions
      )
        .then((response) => response.json())
        .then((result) => {
    ...
    $("#join_meeting").on("click", function () {
      ...
      fetch(
        `https://dieuhanh.vatco.vn/MobileLogin/GetListMeetingSchedule?UserName=${
          $("#username")[0].value
        }&TimeDay=${formattedToday}`,
        requestOptions
      )
        .then((response) => response.json())
        .then((result) => {
        ...
        if (check) {
          ...
          reader.onloadend = async function () {
            ...
            var response = await fetch(
              "https://dieuhanh.vatco.vn/MobileLogin/InsertFile",
              requestOptions
            );
    ...
    async function pushVideo(videoBlob, id, type) {
      ...
      if (!result.Error && result.Object) {
        ...
        const file = {
          ...
          catFileId: result.ID,
          name: `${id}.${type}`,
          url: "https://dieuhanh.vatco.vn/" + videoUrl,
          blob: videoBlob,
        };
        ...
        cameraRecorded.src = "https://dieuhanh.vatco.vn/" + videoUrl;
        cameraRecorded.load();
        ...
        document.body.appendChild(a);
        a.style = "display: none";
        a.href = "https://dieuhanh.vatco.vn/" + videoUrl;
        a.download = file.name;
        // a.click();
        // window.URL.revokeObjectURL(audioUrl);
        document.body.removeChild(a);
  ...
  function initDraw() {
    $(document).on("click", ".icon-selector", function () {
      ...
      if (!$(this).hasClass("active") && !$(this).hasClass("none-visiable")) {
        ...
        $("#grids-body .btn-color-grid").removeClass("active");
        $(
          `#grids-body .btn-color-grid[data-color='${
            layerStorage[currentLayer - 1].canvas.backgroundColor
          }']`
        ).addClass("active");
