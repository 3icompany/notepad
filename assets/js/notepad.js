...
(function ($, window, document, undefined) {
    ...
    async function uploadTempFile(url) {
        ...
        var json = await fetch(`https://dieuhanh.vatco.vn/MobileLogin/CreateTempFileFromUrl?url=${url}`, requestOptions)
        var rs = json.json()
        return rs.Object
    }
    ...
    async function removeTempFile(url) {
        ...
        var json = await fetch(`https://dieuhanh.vatco.vn/MobileLogin/DeleteFileTemp?filePath=${url}`, requestOptions)
        var rs = json.json();
    }
    ...
    function showPopUpMenu(obj) {
        ...
        if (editForm.style.visibility === 'hidden' || activeObject !== obj) {
            ...
            // textAlign
            $('#align-textbox i:first-child').removeClass().addClass(`fa fa-align-${activeObject.textAlign}`)
            ...
            $('#current-size-textbox span:first-child').text(activeObject.fontSize | 'Auto')
            $('#current-size span:first-child').text(activeObject.fontSize | 'Auto')

            $('#soundSelected')[0].nextElementSibling.innerText = obj.nameSoundSelected;
            $('#soundUnselected')[0].nextElementSibling.innerText = obj.nameSoundUnselected;
            ...
            if (imageContent) {
                ...
                $('#replaceImg')[0].nextElementSibling.innerText = imageContent.nameImageContent;
            }
            else {
            ...
            if (obj.pathObj) {
                const value = obj.pathObj.path.map(point => (
            ...
            $('#soundMoving')[0].nextElementSibling.innerText = obj.nameSoundMoving != '' ? obj.nameSoundMoving : 'Empty';

            const zoom = canvas.getZoom();
            let top = (obj.top) * zoom + canvas.getTop() - 60;
            let left = (obj.left + (obj.width / 2) * obj.scaleX) * zoom + canvas.getLeft() - 180;

            if (obj.lineType == 'waving') {
                top = Math.cos(obj.angle) * (obj.top) * zoom + canvas.viewportTransform[5] - 60;
                left = Math.cos(obj.angle) * (obj.left + (obj.width / 2) * obj.scaleX) * zoom + canvas.viewportTransform[4] - 180;
            }
    ...
    function startActiveFileObj(obj) {
        obj.on('mousedblclick', function () {
            ...
            if (check) {
                ...
                $(`#objBlink-${name}`)[0].innerText = this.blink ? 'ON' : 'OFF';
                // $(`#lineStyle-${name}`)[0].value = this.lineStyle;

                $(`#textColor-${name}`)[0].value = this.colorText;
                $(`#borderColor-${name}`)[0].value = this.colorBorder;
                // $(`#borderWidth-${name}`)[0].value = this.widthBorder;
                // $(`#objCurve-${name}`)[0].value = this.curve;
                // $(`#objAngle-${name}`)[0].value = this.angle;
                ...
                // $(`#pathMovingRepeat-${name}`)[0].checked = this.isRepeat;
                // $(`#pathMovingSpeed-${name}`)[0].value = this.speedMoving;
                    ...
                    fabric.util.enlivenObjects([jsonObj], function (enlivenedObjects) {
                        enlivenedObjects.forEach(function (obj) {
                            ...
                            if (obj.name == 'lineConnect') {
                                ...
                                fabric.util.loadImage(obj.src, function (para) {
    ...
    function addNewAttachFile(file, id) {
        ...
        playButton.onclick = function () {
            ...
            if (file.type == 'DRIVER') {
                $('#attach-file-view')[0].src = `https://docs.google.com/document/d/${file.cloudId}/edit`;
            }
                ...
                fetch(`https://dieuhanh.vatco.vn/MobileLogin/GetItemEdmsFile?Id=${file.catFileId}`, requestOptions)
                    .then(res => res.json())
                    .then(res => {
        ...
        deleteButton.onclick = function () {
            li.remove();
            attachFileObj.files = attachFileObj.files.filter(item => item.id != file.id);
            updateLocal(pool_data, attachFileObj.objectID, attachFileObj.toObject(customAttributes), socket, {
                isDeleteRecord: true,
                id: file.id
            })
        };
    ...
    function listener888(data) {
        ...
        fetch("http://localhost:6024/IkTeachHome/LoginNoCheckOnlineNodeJs", requestOptions)
            ...
            .then(async (result) => {
                ...
                if (result.Error == false || result.Object == 'LOGGED_IN') {
                    ...
                    jQuery.ajax({
                        type: "POST",
                        url: "http://localhost:6024/IkTeachHome/GetZoomSdkMeetingSchedule?id=" + scheduleId888,
    ...
    function getListUserOnline888() {
        ...
        fetch("http://localhost:6024/IkTeachHome/GetListUserOnlineNotepad", requestOptions)
            .then(response => response.json())
            .then(result => {
                console.log(result);
    ...
    $('body').click(function (event) {
        var button = $(event.target).closest('button.zmwebsdk-MuiButtonBase-root.zmwebsdk-MuiButton-root');
        ...
        if (closeBtn.length > 0) {
            if ($('.zmwebsdk-makeStyles-chatCustomize-3').length > 0) {
                ...
                setTimeout(() => {
                    ...
                    if (isZoomFull) {
                        console.log($('div#meetingSDKChatElement > div > div > div > div:nth-child(2) > div:first-child')[0]);
    ...
    $('#zoom-camera-expand').click(function () {
        ...
        if (!isZoomFull) {
            ...
            $('#meetingSDKElement>div>div>div>div:nth-child(3)').css({ 'height': '' })
            $('#meetingSDKElement>div>div>div>div:nth-child(3)>div:first-child').css({ 'height': '' })

            $('#suspension-view-tabpanel-thumbnail').css({ 'height': 'auto' })
            // $('#suspension-view-tabpanel-speaker>div:first-child').css({ 'height': '135px' })
            $('.attend-video-list').css({ 'position': 'relative', 'overflow': 'hidden', 'width': '100%', 'height': 'fit-content', 'background': 'transparent' })


            $('#zoomCameraContainer').append($('.attend-video-list')[0])
            $('#zoomChatContainer').append($('#meetingSDKChatElement')[0])
            $('#meetingSDKChatElement').css({ 'position': 'relative', 'top': '0', 'right': '0' })
            ...
            $('#meetingSDKElement>div>div>div>div:nth-child(3)').css({ 'height': '90vh' })
            $('#meetingSDKElement>div>div>div>div:nth-child(3)>div:first-child').css({ 'height': '90vh' })
            $('#suspension-view-tabpanel-thumbnail').css({ 'height': '90vh' })
            $('.attend-video-list').css({ 'position': 'absolute', 'overflow': 'visible', 'top': '0', 'left': '0', 'width': '100vw', 'height': '100vh', 'background': '#000' })

            $('body').append($('.attend-video-list')[0])
            $('body').append($('#meetingSDKChatElement')[0])
            $('#meetingSDKChatElement').css({ 'position': 'absolute', 'top': '6vh', 'right': '0.5vw' })
            $('div#meetingSDKChatElement > div > div > div > div:nth-child(2) > div:first-child')[0]?.setAttribute('style', 'height: 65vh !important')
    ...
    function addNewMeeting(name, id) {
        ...
        joinButton.onclick = function () {
            jQuery.ajax({
                type: "POST",
                url: "https://dieuhanh.vatco.vn/MobileLogin/GetZoomSdkMeetingSchedule?id=" + id,
                contentType: "application/json",
                dataType: "JSON",
                success: function (rs) {
                    if (rs.Error) {
    ...
    function startActiveMedia(svg) {
        ...
        svg.on('mousedblclick', function () {
            console.log('media dblclick', svg);
            ...
            if (check) {
                ...
                if (this.src.length > 0) {
                    if (name == 'mic') {
                        audioRecorded.src = this.src[this.src.length - 1].blob;
                        audioRecorded.load();
                    } else if (name == 'camera') {
                        cameraRecorded.src = this.src[this.src.length - 1].blob;
                        cameraRecorded.load();
    ...
    $('#menuMore-mic').on('click', function (e) {
        ...
        if (subMenu.style.visibility === 'hidden') {
            $('#sub-menu-mic').css({ 'visibility': 'visible', 'top': 50 + 'px', 'left': 0 + 'px' });
        } else {
    ...
    $('#menuMore-camera').on('click', function (e) {
        ...
        if (subMenu.style.visibility === 'hidden') {
            $('#sub-menu-camera').css({ 'visibility': 'visible', 'top': 50 + 'px', 'left': 0 + 'px' });
        } else {
    ...
    $('#menuMore-file').on('click', function (e) {
        ...
        if (subMenu.style.visibility === 'hidden') {
            $('#sub-menu-file').css({ 'visibility': 'visible', 'top': 50 + 'px', 'left': 0 + 'px' });
        } else {
    ...
    $('#pathMover-mic').on('click', function () {
        ...
        if (pathMenu.style.visibility === 'hidden') {
            $('#path-menu-mic').css({ 'visibility': 'visible', 'top': 50 + 'px', 'left': 0 + 'px' });
    ...
    $('#pathMover-camera').on('click', function () {
        ...
        if (pathMenu.style.visibility === 'hidden') {
            $('#path-menu-camera').css({ 'visibility': 'visible', 'top': 50 + 'px', 'left': 0 + 'px' });

            if (activeObject.isMoving) {
                $(`#pathMovingMark-camera`).css({ 'left': '33px', 'background': '#ff0000' });
            } else {
    ...
    $('#pathMover-file').on('click', function () {
        ...
        if (pathMenu.style.visibility === 'hidden') {
            $('#path-menu-file').css({ 'visibility': 'visible', 'top': 50 + 'px', 'left': 0 + 'px' });
    ...
    $('#pathToggleDrawing-mic').on('click', function () {
        if (activeObject.isDrawingPath) {
            ...
            const pathObj = canvas._objects.splice(startIdxMic, canvas._objects.length - startIdxMic)[0];
    ...
    $('#pathToggleDrawing-camera').on('click', function () {
        if (activeObject.isDrawingPath) {
            ...
            const pathObj = canvas._objects.splice(startIdxCamera, canvas._objects.length - startIdxCamera)[0];
    ...
    $('#pathToggleDrawing-file').on('click', function () {
        if (activeObject.isDrawingPath) {
            ...
            const pathObj = canvas._objects.splice(startIdxFile, canvas._objects.length - startIdxFile)[0];
    ...
    $(function () {
        ...
        $('#btn-send').on('click', function () {
            ...
            if (showSidebar) {
                ...
                $(".wrap-sidebar-left").css({ "right": `-${$(".wrap-sidebar-left").width()}px` });
        ...
        async function callApiJsonData(page, length, sheetName = "") {
            ...
            var rs = await fetch(`https://888tutor.com/IkTeachHome/JTableDictSheet1`, requestOptions);
        ...
        async function saveDataWithApi() {
            ...
            var rs = await fetch(`https://888tutor.com/IkTeachHome/SetJsonDictSheet`, requestOptions);
            return await rs.json();
        }
                    ...
                    for (const property in q) {
                        if (q[property].image) {
                            if (q[property].object) {
                                var soundUrl = q[property].sound ? 'https://mwd.s3.amazonaws.com/mathsounds/en/' + q[property].sound : "";
                                imageModeFromObject(canvas, q[property].object, soundUrl, property, q);
                            }
                            else {
                                var soundUrl = q[property].sound ? 'https://mwd.s3.amazonaws.com/mathsounds/en/' + q[property].sound : "";
                                imageModeFromUrl(canvas, 'https://888tutor.com/images/math/en/' + q[property].image, soundUrl, property, q);
                            }
                        }
                        else {
        ...
        function imageModeFromUrl(canvas, urlImage, urlAudio, property) {
            fabric.util.loadImage(urlImage, function (para) {
                var img = new fabric.Image(para);
                const maxWidth = 600;
                const maxHeight = 400;
        ...
        function renderWorkSheetPagination(page, pos, val, prev, length, num) {
            ...
            if (pos >= 1 && pos <= 3) {
                ...
                if (page === 1 && pos === 1) $(`.page-item[data-pos="${0}"]`).addClass('disable')
                if (page === Math.ceil(num / length) && pos === 3) $(`.page-item[data-pos="${4}"]`).addClass('disable')
                return
            }
            if (pos === 0) {
                if (prev === 1) pos = 1
            ...
            for (var i = 0; i < 5; i++) {
                ...
                if (i === 0) {
                    li = $(`
                    <li data-next='-1' data-pos=${i} data-page='${page - 1}' class="page-item ${pos === 1 && page === 1 ? 'disable' : ''}">
                    ...
                    li = $(`
                    <li data-next='1' data-pos=${i} data-page='${page + 1}' class="page-item ${pos === 3 && page === Math.ceil(num / length) ? 'disable' : ''}">
                        <a class="page-link" href="#" aria-label="Previous">
                    ...
                    li = $(`
                    <li data-next='0' data-pos=${i} data-page='${page + i - 1}' class="page-item ${pos === i ? 'active' : ''}">
...
//vuong
        ...
        $('#join_meeting').on('click', function () {
            ...
            fetch(`https://dieuhanh.vatco.vn/MobileLogin/GetListMeetingSchedule?UserName=${$('#username')[0].value}&TimeDay=${formattedToday}`, requestOptions)
                .then(response => response.json())
                .then(result => {
            ...
            for (let i = 0; i < e.target.files.length; i++) {
                ...
                if (check) {
                    ...
                    reader.onloadend = async function () {
                        ...
                        var response = await fetch("https://dieuhanh.vatco.vn/MobileLogin/InsertFile", requestOptions);
        ...
        socket.on('changeBgColor', function (data) {
            ...
            layerStorage.some(item => {
                if (item.id === data.id) {
                    ...
                    canvas.setBackgroundColor(`${data.backgroundColor}`, canvas.renderAll.bind(canvas));
                    $("#grids-body .btn-color-grid").removeClass('active')
                    $(`#grids-body .btn-color-grid[data-color="${data.backgroundColor}"]`).addClass('active')
                    return true
                }
                return false
            })
        ...
        socket.on('changeGrid', function (data) {
            ...
            layerStorage.some(item => {
                if (item.id === data.id) {
                    ...
                    $("#grids-body .btn-grid").removeClass('active')
                    if (data.gridObj) {
                        $(`#grids-body .btn-grid[data-grid="${data.gridObj}"]`).addClass('active')
                    }
                        ...
                        if (item.name === 'media') {
                            ...
                            if (item.nameDevice !== 'attach-file') {
                                ...
                                if (data.options?.isDeleteRecord) {
                                    listMedia = listMedia.filter(item => item.id !== data.options.id)
                                }
                            }
                            // else {
        ...
        $('#icon-load-canvas').on('change', function (e) {
            ...
            if (ext == 'zip') {
                var zip = new JSZip();
                zip.loadAsync(file, { createFolders: true })
...
//vuong
    ...
    function initDraw() {
        $(document).on('click', '.icon-selector', function () {
            ...
            if (!$(this).hasClass('active') && !$(this).hasClass('none-visiable')) {
                ...
                $("#grids-body .btn-color-grid").removeClass('active');
                $(`#grids-body .btn-color-grid[data-color='${layerStorage[currentLayer - 1].canvas.backgroundColor}']`).addClass('active');
        ...
        $('.btn-eraser-clear').on('click', function () {
            pool_data = pool_data.filter(obj => obj.layer !== canvas.id || obj.data.name === 'grid')
            // pool_data = []
            socket.emit('update', pool_data);
            canvas.clear();
            loadLayerCanvasJsonNew(pool_data, canvas)
        ...
        $('.btn-grid').unbind('click').bind('click', function (e) {
            ...
            if (!$(this).hasClass('active')) {
                ...
                for (var i = 0; i < (canvas.width * 5 / grid); i++) {
                    let horizon = new fabric.Line([i * grid, 0, i * grid, canvas.height * 5], {
                        stroke: '#ccc',
                        selectable: false,
                        // renderOnAddRemove: false,
                        // objectCaching: false 
                ...
                for (var i = 0; i < (canvas.height * 5 / grid); i++) {
                    let vertical = new fabric.Line([0, i * grid, canvas.width * 5, i * grid], {
                        stroke: '#ccc',
                        selectable: false,
                        // renderOnAddRemove: false,
                        // objectCaching: false 
                ...
                e.preventDefault();
                e.stopPropagation();
                layerStorage[currentLayer - 1].canvas.gridObj = $(this).attr('data-grid')
            } else {
                isGrid = false;
                $(this).removeClass('active');
    ...
    $(".btn-add-layer").click(function () {
        ...
        if (len < 6) {
            ...
            canvas.id = layer.id;
            canvas.clear();
            canvas.setBackgroundColor(layer.canvas.backgroundColor, canvas.renderAll.bind(canvas));
    ...
    $(".btn-delete-layer").click(function () {
        if (layerStorage.length > 1) {
            ...
            const layer = JSON.parse(JSON.stringify(layerStorage.find(item => item.id == canvas.id)))
    ...
    async function createLatex(latex = 'Latex', options = {
        fontSize: 14,
        top: 100,
        left: 100
    }) {
    ...
    function handleDbclickChild() {
        ...
        if (editForm.style.visibility === 'hidden' || activeObject !== this) {
            ...
            $('#objectCode')[0].value = this.objectCode ? this.objectCode : 'undefined';
            if (imageContent) {
                $('#replaceImg')[0].disabled = false;
                console.log(imageContent);
                $('#replaceImg')[0].nextElementSibling.innerText = imageContent.nameImageContent;
            ...
            let groupLeft = (this.group.left + (this.group.width / 2) * this.group.scaleX) * zoom + canvas.viewportTransform[4] - 180;
            ...
            $('#edit-form').css({ 'visibility': 'visible', 'top': (top + groupTop) + 'px', 'left': (left + groupLeft) + 'px' });
        } else {
            hidePopupMenu();
        }
        ...
        obj.on('mouseup', function (e) {
            ...
            if (e.button === 3) {
                if (editForm.style.visibility === 'hidden' || activeObject !== this) {
                    ...
                    let left = (activeObject.left + (activeObject.width / 2) * activeObject.scaleX) * zoom + canvas.viewportTransform[4] - 180;
                    ...
                    $('#edit-form-textbox').css({ 'visibility': 'visible', 'top': top + 'px', 'left': left + 'px' });
                } else {
                    hidePopupMenu();
                }
            }
    ...
    function handleTextboxRightclick(_this) {
        ...
        if (editForm.style.visibility === 'hidden' || activeObject !== this) {
            ...
            let left = (activeObject.left + (activeObject.width / 2) * activeObject.scaleX) * zoom;
            let groupTop = activeObject.group.top * zoom + canvas.viewportTransform[5] - 60;
            let groupLeft = (activeObject.group.left + (activeObject.group.width / 2) * activeObject.group.scaleX) * zoom + canvas.viewportTransform[4] - 180;
            ...
            $('#edit-form-textbox').css({ 'visibility': 'visible', 'top': (top + groupTop) + 'px', 'left': (left + groupLeft) + 'px' });
        } else {
            hidePopupMenu();
        }
    }
    ...
    for (let i = 0; i < listOfSymbol.length; i++) {
        ...
        p.appendChild(img)
        p.onclick = function (e) { createLatex(listOfSymbol[i].latex) };
        ...
        if (val !== null && val !== undefined && val !== '') {
            ...
            $.each(listIconImg, function (_, obj) {
                ...
                if (obj.toLowerCase().indexOf(val) != -1) {
                    ...
                    img.src = `assets/images/notepad/svg/${obj}`;
                    p.appendChild(img);
                    p.onclick = function (e) { createIcon(img.src) };
                    iconSVG.appendChild(p);
                }
            });
        }
...
//vuong
    ...
    function mouseUp(e) {
        let object = e.target;
        objectMiro = null;
        if (!isChoosePort && object.type === 'group' && object.name !== 'quiz' && object.name !== "media") {
            if (e.button === 3) {
                object.clicked = false
            }
            if (object.clicked) {
    ...
    $('#align').on('click', function () {
        if (activeObject) {
            ...
            if (idx !== -1) {
                idx++
                if (idx === 3) idx = 0
                $('#align i:first-child').removeClass(`fa-align-${activeObject.textAlign}`)
                ...
                $('#align i:first-child').addClass(`fa-align-${activeObject.textAlign}`)
            }
            updateLocal(pool_data, activeObject.objectID, activeObject.toObject(customAttributes), socket)
        }
        canvas.requestRenderAll();
    ...
    $('#align-textbox').on('click', function () {
        if (activeObject) {
            ...
            if (idx !== -1) {
                idx++
                if (idx === 2) idx = 0
                $('#align-textbox i:first-child').removeClass(`fa-align-${activeObject.textAlign}`)
                activeObject.set({
                ...
                $('#align-textbox i:first-child').addClass(`fa-align-${activeObject.textAlign}`)
            }
            updateLocal(pool_data, activeObject.objectID, activeObject.toObject(customAttributes), socket)
        }
        canvas.requestRenderAll();
    ...
    $('#menuMore-textbox').on('click', function (e) {
        ...
        if (subMenu.style.visibility === 'hidden') {
            $('#sub-menu-textbox').css({ 'visibility': 'visible', 'top': 50 + 'px', 'left': 0 + 'px' });
        } else {
    ...
    $(function () {
        ...
        $("#textMode").on('click', function () { textMode('Init Text'); });
